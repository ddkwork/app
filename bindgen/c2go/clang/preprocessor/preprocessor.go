...
func Do(infile, outfile string, conf *Config) (err error) {
	if infile = mylog.Check2(filepath.Abs(infile)); err != nil {
		return
	}
	if outfile = mylog.Check2(filepath.Abs(outfile)); err != nil {
		return
	}
	if conf == nil {
		conf = new(Config)
	...
	base := conf.BaseDir
	if base == "" {
		if base = mylog.Check2(os.Getwd()); err != nil {
			return
		}
	}
