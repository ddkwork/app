...
import (
	...
	ctypes "github.com/ddkwork/app/bindgen/c2go/clang/types"
	"github.com/ddkwork/app/bindgen/c2go/clang/types/scanner"
...
func ParseType(qualType string, conf *Config) (t types.Type, kind int, err error) {
	...
	if t, kind = mylog.Check3(p.parse(conf.Flags)); err != nil {
		return
	}
	if p.tok != token.EOF {
		mylog.Check(p.newError("unexpect token " + p.tok.String()))
	}
	return
}
...
func (p *parser) parseArrays(t types.Type, inFlags int) (ret types.Type, err error) {
	...
	for {
		if tyArr = mylog.Check2(p.parseArray(tyArr, inFlags)); err != nil {
			return
		}
...
func (p *parser) parseFunc(pkg *types.Package, t types.Type, inFlags int) (ret types.Type, err error) {
	...
	args, variadic := mylog.Check3(p.parseArgs(pkg))
...
func (p *parser) parse(inFlags int) (t types.Type, kind int, err error) {
	...
	retry:
		...
		ident:
			...
			case "enum":
				if mylog.Check(p.expect(token.IDENT)); err != nil {
					return
				}
				if t != nil {
			...
			default:
				...
				if t = mylog.Check2(p.lookupType(lit, flags)); err != nil {
					return
				}
				flags = 0
			}
			if flags != 0 {
				...
				if t = mylog.Check2(p.lookupType("int", flags)); err != nil {
					return
				}
				flags = 0
				goto retry
		...
		case token.LBRACK: // [
			if t = mylog.Check2(p.parseArrays(t, inFlags)); err != nil {
				return
			}
		case token.LPAREN: // (
			if t == nil {
			...
			if nstar == 0 {
				...
				if args, variadic = mylog.Check3(p.parseArgs(pkg)); err != nil {
					return
				}
				isFn = true
			} else {
			nextTok:
				...
				case token.LBRACK:
					if tyArr = mylog.Check2(p.parseArrays(ctypes.Void, 0)); err != nil {
						return
					}
					p.expect(token.RPAREN) // )
				case token.IDENT:
			...
			switch p.tok {
			case token.LPAREN: // (
				if t = mylog.Check2(p.parseFunc(pkg, t, inFlags)); err != nil {
					return
				}
			case token.LBRACK: // [
				if t = mylog.Check2(p.parseArrays(t, 0)); err != nil {
					return
				}
			case token.EOF:
			case token.IDENT:
