...
import (
	...
	cast "github.com/ddkwork/app/bindgen/c2go/clang/ast"
...
var builtin_overloads = []overloadFn{
...
func decl_builtin(ctx *blockCtx) {
	...
	mylog.Check(json.NewDecoder(strings.NewReader(builtin_decls)).Decode(&fns))
...
func adjustBigIntConst(ctx *blockCtx, v *gogen.Element, t *types.Basic) {
	...
	val := constant.BinaryOp(v.CVal, token.AND, constant.MakeUint64(mask))
	ival, iok := constant.Uint64Val(val)
