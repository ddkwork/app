...
func compileStringLiteral(ctx *blockCtx, expr *ast.Node) {
	...
	if strings.HasPrefix(value, `L"`) {
		s := mylog.Check2(decodeEscapeString(value[2 : len(value)-1]))

		wstringLit(ctx.cb, s, nil)
		return
	}
	s := mylog.Check2(strconv.Unquote(value))
...
func compileCallExpr(ctx *blockCtx, v *ast.Node) {
	if n := len(v.Inner); n > 0 {
		...
		ellipsis := n > 2 && isVariadic(cb.Get(-n).Type) && isValist(cb.Get(-1).Type)
		if ellipsis {
...
var binaryOps = map[ast.OpCode]token.Token{
	"+": token.ADD,
	"-": token.SUB,
	"*": token.MUL,
	"/": token.QUO,
...
var assignOps = map[ast.OpCode]token.Token{
	"+=": token.ADD_ASSIGN,
	"-=": token.SUB_ASSIGN,
	"*=": token.MUL_ASSIGN,
	"/=": token.QUO_ASSIGN,
...
var unaryOps = map[ast.OpCode]token.Token{
	"-": token.SUB,
	"&": token.AND,
	"~": token.XOR,
	"!": token.NOT,
