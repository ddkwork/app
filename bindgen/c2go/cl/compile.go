...
import (
	...
	"github.com/ddkwork/app/bindgen/c2go/clang/cmod"
	"github.com/ddkwork/app/bindgen/c2go/clang/types/parser"
...
func NewPackage(pkgPath, pkgName string, file *ast.Node, conf *Config) (pkg Package, err error) {
	...
	pkg.pi = mylog.Check2(loadFile(pkg.Package, conf, file))
	return
}
...
func compileFunc(ctx *blockCtx, fn *ast.Node) {
	...
	if body != nil {
		...
		f := mylog.Check2(pkg.NewFuncWith(ctx.goNodePos(fn), fnName, sig, nil))
...
var tyPVPA = types.NewPointer(types.NewArray(ctypes.UnsafePointer, 2))
